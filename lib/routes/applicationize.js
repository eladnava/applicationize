...
function* buildCrxConfig(ctx) {
    ...
    crxConfig.title = getCrxTitle($, crxConfig, input);
    ...
    crxConfig.frameColor = getCrxFrameColor(input);
...
function getCrxTitle($, crxConfig, input) {
...
function getCrxFrameColor(input) {
    // Extract frame color from input
    var frameColor = input.field.frameColor;
    ...
    if (!validator.isHexColor(frameColor)) {
        throw new Error('Please provide a valid hex color value to customize the frame color.');
    }
...
function customizeBackgroundScript(crxConfig, crx) {
    ...
    if (crxConfig.frameColor) {
        ...
        chromeAppWindow.frame.color = crxConfig.frameColor;
    }
    ...
    // Inject chrome app window params into background.js file (replace {inject-app-window-params} with stringified JSON)
    js = js.replace(/{inject-app-window-params}/, JSON.stringify(chromeAppWindow));
